 ...
 * 
 * BEComboBoxUI.java at 2015-2-1 20:25:38, original version by lornwolf.
...
//                    .setFocusedThikness(NLLookAndFeel.comboxFocusedThikness));
    ...
    //copy from parent and modified by lornwolf
    /* (non-Javadoc)
...
//        comboBox.setMaximumRowCount(8);//这个最大行可以起效，但似乎它的行高指的是一个固定值而不是计算值，像本LNF里因cell本身行高就很高
        ...
        if (parent != null) 
        {
            ...
            if (parent != null && parent instanceof JScrollPane) 
                LookAndFeel.installBorder((JScrollPane)parent, "ComboBox.scrollPaneBorder");//*~ 注：这个属性是lornwolf仿照JTabel里的实现自已加的属性
        }
    }
...
//    }
    ...
    //* Menu.menuPopupOffsetX等4个属性可设置以备对坐标进行调整，BeautyEye LNF中由lornwolf
    ...
    //* 此方法由lornwolf于2012-10-12加入：重写父类方法的目的是使得默认的
