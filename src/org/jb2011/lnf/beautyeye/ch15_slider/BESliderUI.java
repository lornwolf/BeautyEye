 ...
 * 
 * BESliderUI.java at 2015-2-1 20:25:38, original version by lornwolf.
...
public class BESliderUI extends BasicSliderUI {
    ...
    /** 水平Slider的Thumb高度. */
    protected final int THUMB_HEIGHT_HORIZONAL = 7;// 此属性可提取为Ui属性，方便以后配置（大小应是NP图的最小高度，最大值得看JSlider的高度了）
    
    /** 垂直Slider的Thumb宽度. */
    protected final int THUMB_WIDTH_VERTICAL = 7;// 此属性可提取为Ui属性，方便以后配置（大小应是NP图的最小高度，最大值得看JSlider的高度了）
    ...
    public void paintTrack(Graphics g) {        
        ...
        if (slider.getOrientation() == JSlider.HORIZONTAL) {
            ...
            if (slider.isEnabled()) {
                ...
                //轨道（填充到当前刻度值处）
                // BUG：当前有个bug，即在SwingSets2演示中，当thumb高度较小时，轨道半圆形被画出，这可能父类中thumbRect中计算有部碮同，有时间再研究吧，官方以后版本或许能解决哦
